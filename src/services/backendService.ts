...
class BackendService {
  ...
  async getPageContent(url: string): Promise<PageContentData> {
    return (
      (await this.get(`/api/content/${url}`))?.page || {
        pageUrl: url,
        heading: "404",
  ...
  async deleteOldFiles(days: number): Promise<{ deleted: number }> {
    return await this.get(`/api/files/deleteold/${days}`);
  }
    ...
    return (
      (
        await this.get(
          offset === undefined
            ? `/api/admin/allusers`
            : `/api/admin/allusers/${offset}/${limit || 10}`
        )
      )?.users.sort((a: UserDisplayData, b: UserDisplayData) => {
        const p1 = a.id;
        const p2 = b.id;
        if (p1 < p2) return 1;
        if (p1 === p2) return 0;
  ...
  async uploadFreeVideoByAdmin(file: File): Promise<IUploadResponse> {
    const formData = new FormData();
    formData.append("file", file); // ← ключ должен быть "file"

    const endpoint = `${API_URL}/api/video/upload`;
  ...
  async downloadFreeVideoByAdmin(filename: string): Promise<IUploadResponse> {
    return await this.get(`/api/file/free/${filename}`);
  }
  ...
  async createOrder(
    items: OrderItemData[],
    cmr_data: CMRDeliveryData,
    shoes?: ItemDataShoes[],
    clothes?: ItemDataClothes[]
  ) {
    return await this.post(
      "/api/createpayments",
      { items, cmr_data, shoes, clothes },
      "POST"
    );
  }
